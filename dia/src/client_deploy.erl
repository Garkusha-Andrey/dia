...
start(LocalIP, RemoteIp) ->
	error_logger:info_msg("Start of Erlang client_deploy~n"),
	add_clients('c', LocalIP, RemoteIp, 5).

add_clients(Pattern, LocalIP, RemoteIp, Count) ->
	error_logger:info_msg("Adding ~p clients: ~n", [Count]),
...
continues_call(Name, Realm, Host, IntervalMS) ->
	error_logger:info_msg("lient:call(~p ~p ~p ~p ~n", [Name, user, Realm, Host]),
	client:call(Name, user, Realm, Host),
	timer:sleep(IntervalMS)
	.

%% 'c1','ex.ru','127.0.0.1','127.0.0.1','3911'
%add_clients_pattern(_NamePattern, _Realm, _LocalIP, _RemoteIP, _Port, _From, _To) ->
%	ok;
