...
start({T, Ref}, #diameter_service{capabilities = Caps}, Opts) ->
    nfv_diameter_tcp_sup:start(),  %% start tcp supervisors on demand
    {Mod, Rest} = split(Opts),
    Addrs = Caps#diameter_caps.host_ip_address,
    Arg = {T, Ref, Mod, self(), Rest, Addrs},
    nfv_diameter_tcp_sup:start_child(Arg).
...
i({T, Ref, Mod, Pid, Opts, Addrs})
  when T == accept;
    ...
    %% returns.
    {ok, MPid} = nfv_diameter_tcp_sup:start_child(#monitor{parent = Pid}),
...
init(accept = T, Ref, Mod, Pid, Opts, Addrs) ->
    ...
    error_logger:info_msg("[agarkush] diameter_tcp:: init accept. Opts: ~p~n"
...
accept_peer(Mod, Sock, Matches) ->
    {RAddr, _RPort} = ok(peername(Mod, Sock)),
...
%% ... or not: start one.
l([], LRef, T) ->
    {ok, _, AS} = nfv_diameter_tcp_sup:start_child({listen, LRef, self(), T}),
    AS.
