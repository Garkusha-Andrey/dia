...
#######################################################################
# Geting vSwitch instance ip address from cloudify-management-network #
# for instance configuration                                          #
#######################################################################
while [[ -z "$INST_IP" ]]
do
    INST_IP=$(curl -s -H "X-Auth-Token: ${TOKEN_CLY}" http://192.168.203.210:8774/v2.1/c5cb11f2e7604f30bae0b61cec0086f7/servers/${vSwitch_id} | jq '.server.addresses."cloudify-management-network"[].addr' | tr -d '"')
    sleep 1
done
