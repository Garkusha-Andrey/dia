...
if [ $APPLICATION = "" ]; then
	...
	then
		...
		then
			...
			setsid erl -detached -name $Enode -kernel error_logger \{file,\"/home/nfvpilot/`echo $Enode`.log\"\} -s boot start -s controller_app change_configuration \
		...
		then
			...
			then
				...
				then
					...
					Enode=controller`echo $CPRIO`@`echo $LOCALIP`
					setsid erl -detached -name $Enode -kernel error_logger \{file,\"/home/nfvpilot/`echo $Enode`.log\"\} -s boot start $RNODE -s controller_app change_configuration \
					    $OVSIntIp $OVSIntMask $PublicIp $PublicMask $OVSMac $ExtGwMac \
					    -setcookie 'ABCD' \
					    -config ../config/controller`echo $CPRIO`.config >/dev/null 2>&1 < /dev/null
				else
					Enode=controller`echo $CPRIO`@`echo $LOCALIP`
					setsid erl -detached -name $Enode -kernel error_logger \{file,\"/home/nfvpilot/`echo $Enode`.log\"\} -s boot start -s $APPLICATION change_configuration \
					    $OVSIntIp $OVSIntMask $PublicIp $PublicMask $OVSMac $ExtGwMac \
					    -setcookie 'ABCD' \
					    -config ../config/controller`echo $CPRIO`.config >/dev/null 2>&1 < /dev/null
				fi
			...
			Enode=controller`echo $CPRIO`@`echo $LOCALIP`
					setsid erl -detached -name $Enode -kernel error_logger \{file,\"/home/nfvpilot/`echo $Enode`.log\"\} -s application start inets -s boot start -s $APPLICATION change_configuration \
					    $OVSIntIp $OVSIntMask $PublicIp $PublicMask $OVSMac $ExtGwMac \
		...
		if [ "$APPLICATION" = "diameter" ]; then
			...
			then
				...
				setsid erl -detached -name $DEnode \
				 -kernel error_logger \{file,\"/home/nfvpilot/`echo $DEnode`.log\"\}  -s boot start $RNODE \
